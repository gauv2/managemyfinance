...
function renderCategorizationFlags(container: HTMLElement, plugin: FinancePlugin): void {
	...
	const card = container.createDiv({ cls: "fp-card fp-flags-card" });
	const head = card.createDiv({ cls: "fp-card-head-row" });
	head.createEl("h3", { text: "Possible categorization issues" });
	head.createDiv({ cls: "fp-card-head-label", text: `${flags.length} FOUND` });
	card.createEl("p", {
		cls: "fp-flags-desc",
...
function renderFlagRow(parent: HTMLElement, flag: CategorizationFlag): void {
	...
	top.createSpan({ cls: "fp-flag-row-name", text: flag.key });
	top.createSpan({
	...
	shown.forEach((o) => {
		outliersLine.createSpan({
			cls: "fp-flag-outlier",
			text: `${o.transaction.date} · ${formatEUR(o.transaction.amount)} → "${o.categoryName}"`,
		});
	});
