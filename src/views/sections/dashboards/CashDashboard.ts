...
export function renderCashDashboard(container: HTMLElement, plugin: FinancePlugin, account: Account): void {
	...
	const count = store.transactions.filter((t) => t.accountId === account.id).length;
	...
	statTile(tiles, { label: "Net this year", value: currentYear ? formatEUR(currentYear.net) : "—", iconName: "trending-up" });
