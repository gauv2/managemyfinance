...
export function renderCardsSection(container: HTMLElement, plugin: FinancePlugin): void {
	...
	function renderCardTile(parent: HTMLElement, card: Card, accountName: string): void {
		const tile = parent.createDiv({ cls: "fp-card-tile" });
		const head = tile.createDiv({ cls: "fp-card-tile-head" });
		head.createDiv({ cls: "fp-card-tile-account", text: accountName.toUpperCase() });
		const detailBtn = head.createEl("button", { cls: "fp-btn-icon fp-btn-ghost", attr: { "aria-label": "Card details" } });
		icon(detailBtn, "more-horizontal");
		detailBtn.addEventListener("click", () => new CardDetailModal(plugin.app, plugin, card, accountName, () => render()).open());
