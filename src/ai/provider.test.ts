...
describe("callModel over the CLI", () => {
	it("refuses a file rather than asking the model to identify a receipt it was never shown", async () => {
		await expect(callModel({ ...request, attachments: [{ mediaType: "image/png", data: "QUJD" }] }, { provider: "cli" })).rejects.toThrow(
			/CLI provider can't do/
		);
	});
});
