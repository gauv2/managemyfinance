...
function row(container: HTMLElement, label: string, value: string, opts?: { sensitive?: boolean }): void {
...
export class CardDetailModal extends Modal {
	...
	private render(): void {
		...
		if (this.card.cardholderName) row(body, "Cardholder", this.card.cardholderName);
		if (this.card.number) row(body, "Card number", groupedNumber(this.card.number), { sensitive: true });
		else if (this.card.last4) row(body, "Card number", `•••• •••• •••• ${this.card.last4}`, { sensitive: true });
		if (this.card.expiryMonth && this.card.expiryYear) {
			row(body, "Expires", `${String(this.card.expiryMonth).padStart(2, "0")}/${String(this.card.expiryYear % 100).padStart(2, "0")}`);
		}
